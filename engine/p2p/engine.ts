...
export function createP2PEngine<C, S, A, R, V>(options: EngineOptions<C, S, A, R, V>) {
  ...
  async function view(entry: Entry): Promise<RoundView<V, R>> {
    ...
    return {
      ...
      allowed: {
        ...
        cancel:
          !entry.play && !entry.finish && !match && !entry.cancelled && !entry.cancelRequested,
      },
  ...
  async function run(
    ...
    if (operation !== 'cancel') await resume(path, at);
    await deadlines.update<Entry>(path, (entry) => {
      if (duplicate(entry, input.commandId, fingerprint)) return { value: entry };
      if (operation === 'cancel') {
        if (entry.play || entry.finish || matched(entry)) throw new EngineError('cannot_cancel');
        return {
          value: acknowledge(
            { ...entry, cancelRequested: true, cancelled: !entry.receipt || entry.cancelled },
            input.commandId,
            fingerprint,
            at,
          ),
  ...
  return {
    ...
    cancel: (actor: Actor, input: Command) => run(actor, input, 'cancel'),
